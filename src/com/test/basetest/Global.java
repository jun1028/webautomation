...
public class Global
{
	...
	{
		g_Action = new Actions(Global.getDriver());
	}
	...
	protected static void setTimeOut(int numTimeOunt)
	{
		...
		Global.getDriver().manage().timeouts().implicitlyWait(numTimeOunt, TimeUnit.SECONDS);
	}
	...
	public static void initDriver()
	{
		try
		{
			...
			System.setProperty("webdriver.chrome.driver", Global.getHomePath() + "\\chromedriver.exe");
	...
	public static void initHttpClient()
	{
		try
		{
			org.apache.log4j.Logger mylog = org.apache.log4j.Logger.getLogger("");
			mylog.setLevel(Level.OFF);
			Global.g_httpclient = HttpClients.createDefault();
			Global.g_requestConfig = RequestConfig.custom().setSocketTimeout(10000).setConnectTimeout(10000).build();
...
//			Report.r_Local_OS = System.getProperty("os.name").toString() + " " + realArch;
			...
			for(int j=0;j<=objSheet.getLastRowNum();j++)
			{
				...
				for(int k=0;k<=objRow.getLastCellNum();k++)
				{
					if(objRow.getCell(k) != null && !objRow.getCell(k).getStringCellValue().isEmpty())
					{
						...
						{
							String strData = objRow.getCell(k).getStringCellValue().replaceAll("\\{\\{SX\\}\\}", Global.getEnv().getProperty("suffix"));
							System.out.println("Data replaced to " + strData);
							objRow.getCell(k).setCellValue(strData);
						}
					}
	...
	protected static void initChartType()
	{
		...
		try
		{
			System.out.println(Global.getHomePath() + "//MJ_LIB//src//lib//ChartTypeID.xml");
			File file = new File(Global.getHomePath() + "//MJ_LIB//src//lib//ChartTypeID.xml");
